async function showPics(idx) {
  ...
  $("#trppStartEndDate")[0].innerHTML =
    vals[trpHdrs.indexOf("Start Date")].slice(0, -5) +
    " - " +
    vals[trpHdrs.indexOf("End Date")].slice(0, -5);
  $("#trppArrIdx").val(idx);

  $("#trppRtnToPage")[0].setAttribute(
    "onclick",
  ...
  var trpDtl = buildLocArr(
    JSON.parse(vals[trpHdrs.indexOf("Destination Detail")])
  );
...
function buildLocArr(trpDtl) {
  ...
  for (var i = 0; i < trpDtl.length; i++) {
    ...
    if (date != brkDate) {
      ...
      trp.push([
        "<div class='text-start my-2'><span class='text-primary h3'>" +
          dispDate +
          "</span></div>",
...
async function listPhotos(strDate, endDate) {
  ...
  for (var j = 0; j < mediaArr.length; j++) {
    ...
    var fnameDate =
      media.filename.toLowerCase() + media.mediaMetadata.creationTime;
    ...
    console.log(
      ...
      prevDte,
      DateTime.fromISO(prevDte).toFormat("ccc L/d")
    );

    if (mediaDate != dte) {
      if (medArr.length > 0) {
        var ele = $tblPics.clone();
        ele.find("#trppDate")[0].innerHTML =
          DateTime.fromISO(prevDte).toFormat("ccc L/d");
...
async function embed_google_media(mediaItems, id, type = "grid", height = 240) {
  ...
  for (let i = 0; i < mediaItems.length; i++) {
    ...
    media_item.aspect_ratio =
      mediaItems[i].mediaMetadata.width / mediaItems[i].mediaMetadata.height;

    media_item.product_url = mediaItems[i].productUrl;
    media_item.createTime = mediaItems[i].mediaMetadata.creationTime.replace(
      /:/g,
      ""
    );
...
function MediaGrid(container, mediaArr, max_height) {
  ...
  function _calc_rows() {
    ...
    for (let object of media_objects) {
      ...
      if (current_aspect_ratio > min_aspect) {
        // this row is full we can cap it
        let row_item = {};
  ...
  function _populate_container(rows) {
    ...
    for (let row of rows) {
      let image_width =
        container.clientWidth - space_between_media * (row.items.length - 1);
...
class MediaObject {
  ...
  _update_position(x, y, height) {
    this.dom_object.style = `width:${parseInt(
      this.aspect_ratio * height
    )}px;height:${parseInt(height)}px;transform:translate(${parseInt(
      x
    )}px,${parseInt(y)}px)`;
  }
